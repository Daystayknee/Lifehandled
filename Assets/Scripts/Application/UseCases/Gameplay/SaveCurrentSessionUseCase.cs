    ...
    public class SaveCurrentSessionUseCase
    {
        ...
        public void Execute(GameSessionContext context)
        {
            ...
            envelope.vs01State.npcs = context.npcs.Select(n => new Vs01NpcState
            {
                ...
                voiceType = n.profile.voiceType,
                speechStyle = n.profile.speechStyle,
                clothingStyleId = n.profile.clothingStyleId,
                occupationCategory = n.profile.occupation?.category ?? Domain.Common.OccupationCategoryType.Service,
                occupationJobId = n.profile.occupation?.jobId ?? "barista",
                occupationSchedulePreset = n.profile.occupation?.schedulePreset ?? "day_shift",
                occupationDailyIncome = n.profile.occupation?.dailyIncome ?? 10,
                occupationReputationImpact = n.profile.occupation?.reputationImpact ?? 1f,
                occupationSocialCircleRadius = n.profile.occupation?.socialCircleRadius ?? 1f,
                upbringing = n.profile.lifeHistory?.upbringing ?? "ordinary_town",
                educationLevel = n.profile.lifeHistory?.educationLevel ?? Domain.Common.EducationLevelType.HighSchool,
                hometown = n.profile.lifeHistory?.hometown ?? "founders_town",
                pastRelationshipNotes = n.profile.lifeHistory?.pastRelationshipNotes ?? new List<string>(),
                previousJobs = n.profile.lifeHistory?.previousJobs ?? new List<string>(),
                traumaTags = n.profile.lifeHistory?.traumaTags ?? new List<string>(),
                favoriteFoodItemIds = n.profile.preferences?.favoriteFoodItemIds ?? new List<string>(),
                favoriteActivityIds = n.profile.preferences?.favoriteActivityIds ?? new List<string>(),
                preferredClothingStyleId = n.profile.preferences?.preferredClothingStyleId ?? n.profile.clothingStyleId,
                musicTasteId = n.profile.preferences?.musicTasteId ?? "pop",
                hobbyIds = n.profile.preferences?.hobbyIds ?? new List<string>(),
                personalityTraits = n.profile.personalityTraits,
                emotionalTraits = n.profile.emotionalTraits,
                socialTraits = n.profile.socialTraits,
